...
import math
from .sdpa_gqa import sdpa_gqa


def rms_layernorm(hidden: torch.Tensor, weight: torch.Tensor, eps: float):
    old_dtype = hidden.dtype
    variance = hidden.to(torch.float32).pow(2).mean(dim=-1, keepdim=True)
    eps_tensor = torch.tensor(eps, dtype=variance.dtype, device=variance.device)
    hidden = (hidden * torch.rsqrt(variance + eps_tensor)).to(old_dtype)
...
class MiniCPMLongRoPE(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # 使用 torch.index_select，它更明确地映射到 ONNX 的 Gather 节点
        # position_ids 需要是 1D 张量，所以我们先把它展平
        original_shape = position_ids.shape
        flat_position_ids = position_ids.view(-1)
        
        cos = torch.index_select(self.cos_cached, 0, flat_position_ids)
        sin = torch.index_select(self.sin_cached, 0, flat_position_ids)
        
        # 恢复原始形状
        cos = cos.view(original_shape + (self.cos_cached.shape[-1],))
        sin = sin.view(original_shape + (self.sin_cached.shape[-1],))
...
class MiniCPMAttention(nn.Module):
    ...
    def forward_step(
        ...
        # Concatenate past keys and values unconditionally
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward_step(
        ...
        return hidden_states, next_keys, next_values
...
class MiniCPMModel(nn.Module):
    ...
    def forward(
        ...
        Returns:
            ...
            next_keys: Tensor(batch_size, num_layers, num_heads, seq_length, head_dim)
            next_values: Tensor(batch_size, num_layers, num_heads, seq_length, head_dim)
        """
        ...
        # Collect keys and values from all layers
        all_keys = []
        all_values = []
        ...
        for decoder_layer in self.layers:
            ...
            all_keys.append(this_cache[0])
            all_values.append(this_cache[1])
            
        # Stack all keys and values into tensors
        # dim=1 means stacking along the "layer" dimension
        next_keys = torch.stack(all_keys, dim=1)
        next_values = torch.stack(all_values, dim=1)
    ...
    def forward_step(
        ...
        Args:
            ...
            position_id: Tensor(batch_size,)
            past_keys: Optional[Tensor(batch_size, num_layers, num_heads, past_seq_length, head_dim)]
            past_values: Optional[Tensor(batch_size, num_layers, num_heads, past_seq_length, head_dim)]
        Returns:
            hidden_states: Tensor(batch_size, hidden_size)
            next_keys: Tensor(batch_size, num_layers, num_heads, 1, head_dim)
            next_values: Tensor(batch_size, num_layers, num_heads, 1, head_dim)
        """
        ...
        for i, decoder_layer in enumerate(self.layers):
            # Index past keys and values for this layer unconditionally
            layer_past_keys = past_keys[:, i]
            layer_past_values = past_values[:, i]
            ...
            # Collect new keys and values for this layer (only the last token)
            all_keys.append(layer_next_keys[:, :, -1:, :])
            all_values.append(layer_next_values[:, :, -1:, :])
